...
export const getConsultantController = async (
  ...
  res.status(200).json({
    message: USERS_MESSAGES.GET_CONSULTANTS_FOR_ADMIN_SUCCESSFULLY,
    result
  })
}

export const editStatusConsultantController = async (
  req: Request<ParamsDictionary, any, EditStatusUserReqBody, EditReqQuery>,
  res: Response,
  next: NextFunction
) => {
  const { id } = req.params
  const { status } = req.body
  await usersServices.editStatusConsultant(id, status)
